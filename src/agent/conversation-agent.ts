...
### RPG 成长
enemy:death → LevelUp +XP → 累积到升级阈值 → levelup:levelup → 属性成长(hp+10, attack+2, defense+1)
enemy:death → EnemyDrop → 按掉落表概率掉落(药水/金币/装备)

### 伤害链
collision:damage → Shield(消耗充能) → Health(扣血) → IFrames(无敌闪烁) → Knockback(击退) → Lives(血量归零时-1命) → lives:zero → GameFlow 结束
